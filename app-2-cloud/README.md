...
Most of the steps that you use to create a service instance with AppToCloud are the same as those you use to create a standard Java Cloud Service instance. However, there are some additional steps:
...
Follow the steps below, as also refer to the Oracle documentation [Creating an Oracle Java Cloud Service Instance with AppToCloud](http://docs.oracle.com/en/cloud/paas/java-cloud/jscug/creating-oracle-java-cloud-service-instance-apptocloud.html) for more details and clarification!
...
![](images/13.app2cloud.storage.png)
 ...
 - **Service Release:** Select the **Recommended** 12c release
 - **Service Edition:** Enterprise Edition
 - **Compute Shape:** use the default, small instance
 - **SSH Public Key:**  use the same one created for the Database Cloud Service or if you don't have or want to create different keypair select Create a New Key option and download the newly generated keypair for later usage. 
 - **Cluster Size:** tleave the 2 which is based on the JSON configuration file
 - **Domain Partitions:** default
 - **Username:**  WebLogic Administrator username
 - **Password:** WebLogic Administrator's password
 - **Enable access to Administration Consoles:** checked
 - **Do not Deploy Sample Application** 
 - **Do not create and configure a Coherence Data Tier** 
 - **Database Configuration** for For Oracle Required Schema
	 - **Database Name:** use the same what was prepared previously for Petstore demo application (i.e. *techcoDB*)
	 - **PDB Name:** default (usually PDB1)
	 - **Administrator Username:** DBA admin to create repository schema for Java Cloud Service instance, i.e. *sys*
	 - **Password:** DBA admin password you provided during Database Cloud Service creation.
	 - **For Application Schema:** do not Add
 - **Provision Load Balancer:** selectt: **Yes**
...
On the **Additional Service Details** screen, select the first **Application Data Source**: *PETSTORE*
Complete the following parameters:
 - DBCS Instance: select the Database Cloud Service DBCS Instance for Petstore application and which was also selected to host JRF required schemas  (i.e. *techcoDB*).
...
If you are satisfied with your choices click **Create**.
